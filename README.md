...
3. 下载配置选项  
    ...
    3. 下载线程：单文件分段并发下载的连接数（默认 4 线程，可选 1/2/4/8/16）。  
        服务器不支持 Range 时会自动回退单线程，可放心调整。
    4. Debug 模式：开启后把日志写入应用数据目录 `debug/asmr_downloader.log`，  
        方便在 Windows 等平台排查问题。
    5. 启用代理：  
...
- **基于「同名字幕是否存在」自动判断**：若某个音轨已有 `.lrc` / `.vtt` / `.srt` 官方字幕，则跳过 AI 翻译（官方字幕够用，不浪费算力）；仅对**没有任何字幕的音轨**调用 AI，且只传缺字幕的音轨文件清单（不会重复翻译已有官方字幕的音轨）。输出只保留 **lrc** 格式。
- **用法**：
  1. **推荐：点「安装引擎」一键安装**（作品库页 AI字幕 控件）——选显卡变体（NVIDIA CUDA / AMD ROCm）与任务、自选安装目录（需 ≥ 12 GB 剩余空间），应用自动从上游 GitHub Release 下载运行时（分卷下载/断点续传/SHA-256 校验/解压）与 Whisper 模型（HuggingFace，国内自动回退 hf-mirror），装完自动配置。中途可取消，重新安装从断点继续。
  2. **手动模式**：自行下载 ChickenRice 对应 release（按显卡选 CUDA/AMD 版本）解压到本地，在 **AI字幕** 控件点「选择脚本」选 `.bat` 启动脚本（如 `运行(翻译)(GPU).bat`，翻译/转录与设备由所选 bat 决定）或 `infer.exe`（此时用旁边的下拉选任务与设备）。
  3. 在作品列表中勾选作品后点工具栏 **字幕**（或点行内的字幕按钮）生成；勾选 **自动** 则下载完成后自动翻译。批量勾选多个作品会**聚合为一次进程调用**（模型只加载一次，进度为跨作品总进度；中途取消终止整批）。
- 生成的字幕与音轨同名（`xxx.lrc`），Navidrome 整理时会被自动采纳并内嵌为歌词标签。
- **vtt 转 lrc**：作品行内「歌词」按钮把 `.vtt` 字幕批量转为同名 `.lrc`（已有 `.lrc` 不覆盖）。
- **平台**：AI 字幕翻译**仅支持 Windows**（ChickenRice 官方只提供 Windows exe/bat；macOS 不支持，相关控件会自动禁用）。
- **进度与日志**：工具栏实时显示百分比/文件数/当前文件名（模型加载阶段显示「启动中…」）；点工具栏日志图标可实时查看 ChickenRice 的完整输出（运行结束后仍可查看，便于排查）。取消按钮会终止整个进程树。若提示「未找到需要处理的文件」，说明音频格式不在支持列表内（已与官方 bat 的后缀列表对齐）。
- **本地参考与第三方声明**：`resource/Faster-Whisper-TransWithAI-ChickenRice` 仅为本地参考副本，不随仓库分发（已在 `.gitignore` 中排除）；内置安装器也是按需从上游下载，不随本应用分发。第三方版权声明见 [THIRD-PARTY.md](THIRD-PARTY.md)。
- 注意：首次运行会下载/加载 Whisper 模型，较耗时、占用 GPU 显存；进程可通过取消按钮中断。

## 功能特色

1. 多线程下载：  
    单个文件按「下载线程」配置分成多段并发下载（默认 4 线程），每段至少 1 MiB；  
    服务器不支持 Range 时自动回退单线程，兼容性不受影响。  
    每段独立断点续传，中断后下次从各分段断点继续。
2. 断点续传：  
...
1. 下载页可配置「下载线程」提高单文件下载速度；仍可通过打开多个应用手动多进程下载不同作品🙃
