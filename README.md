...
- **进度与取消**：工具栏实时显示「第 n/m 个文件」与当前文件名；取消按钮会终止整个进程树（bat 模式连子进程一并终止）。若提示「未找到需要处理的文件」，说明音频格式不在支持列表内（已与官方 bat 的后缀列表对齐）。
- **本地参考**：`resource/Faster-Whisper-TransWithAI-ChickenRice` 仅为本地参考副本，不随仓库分发（已在 `.gitignore` 中排除）。
